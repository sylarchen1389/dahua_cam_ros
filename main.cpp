...
enum cameraTypr{leftRaw=0,rightRaw,leftDetect,rightDetect};
...
// 测试连接，打开后不会取帧
    ...
    /*设置主动曝光*/
    setExposureTime(cameraSptr, 20000,true);

    /*设置白平衡，三个值*/
   // setBalanceRatio(cameraSptr, double dRedBalanceRatio, double dGreenBalanceRatio, double dBlueBalanceRatio)；
    ...
    /* 创建取pub线程 */
    ros::init(argc,argv,"dahua_cam");
    ros::NodeHandle nh;
    Dahua::Memory::TSharedPtr<PubliserStreamRetrieve> streamThreadSptr(new PubliserStreamRetrieve(streamPtr,nh));
